...
5. Stage 4 — `sd-housekeeping`: invoke housekeeping exactly once. Its gate
   ...
   `--finish-work-head "$(git rev-parse HEAD)"`. That exact-head handoff allows the executable gate
   to own the one post-finish Obsidian KB refresh for repositories that already
   have a KB, perform the merge, and report the post-merge state. Housekeeping
   remains its only owner and `sd-ship` relays that outcome. Never pass the
   handoff flag when finish-work blocked or its commits are not pushed and
   green.

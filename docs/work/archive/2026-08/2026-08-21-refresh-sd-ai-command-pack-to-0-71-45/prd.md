...
- Run the manifest-ordered preparation and check commands for this consumer (`npm ci`, then `npm test`, `npm run check`, and `npm run validate:metadata`) before the local gate.
...
- [x] <!-- verify: lane-evidence id=check-command --> `npm test`, `npm run check`, and `npm run validate:metadata` all pass.

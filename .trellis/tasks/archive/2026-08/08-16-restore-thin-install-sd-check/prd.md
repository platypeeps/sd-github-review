...
Delivered upstream, not here — as the requirements demanded. The fix shipped as
`platypeeps/sd-ai-command-pack#482` in pack **0.71.26**, and this repository was
refreshed to it (`.sd-ai-command-pack/manifest.json`). Nothing was restored
under `scripts/`.
...
2. All five formerly-`unavailable` rows now report `passed`:
   ...
   the `unavailable` state is what "skipped" looks like, and none remain.
3. and 4. Covered by upstream tests in #482, which extended the existing
   sibling-resolution suite to the thin layout — including the case the review
   caught as missing, `knowledge.obsidian-kb` present in the machine install,
   alongside the pre-existing absent-from-both case that must stay
   `unavailable`. 25 tests green upstream at merge.
5. Proven the strong way: `sd-review scope=pr` did not merely pass the
   capability phase, it completed **five** live routed rounds against PR #93
   and reached a terminal `status: ready` report with remote observation
   materialized.

The `.sd-ai-command-pack/bin/sd-ai-command-pack-review-layout.py` seam suggested
in the notes is the one the upstream fix used.
